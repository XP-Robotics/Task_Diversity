...
def apply(profile: SourceProfile) -> None:
    ...
    This is the schema-pipeline copy: the diversity engine (``metrics``) is not
    shipped here, so diversity aliases are merged into the config but the
    diversity-canon rebuild is skipped when ``metrics`` is unavailable. Schema
    conformance never consults the diversity canons, so this is a no-op for it.
    """
    ...
    if profile.diversity_aliases:
        ...
        try:
            from . import metrics  # diversity engine; absent in the schema pipeline
        except ImportError:
            pass
        else:
